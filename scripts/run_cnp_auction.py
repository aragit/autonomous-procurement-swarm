...
async def main():
    ...
    for i, bid in enumerate(result["valid_bids"], 1):
        print(
            f"   {i}. {bid['supplier_id']}: ${bid['unit_price']:,.2f} | "
    ...
    if len(result.get("shortlist", [])) > 1:
        print(f"\n{'=' * 70}")
        print("🤝 BILATERAL BARTERING PHASE")
        print(f"Shortlist: {len(result['shortlist'])} suppliers")
        print(f"{'=' * 70}")

        bilateral = await orchestrator.run_bilateral_bartering(
            session_id=session_id,
            buyer=buyer,
        ...
        if bilateral["success"]:
            deal = bilateral["best_deal"]
            print(f"\n🏆 POST-BARTER AWARD: {deal['supplier_id']}")
            print(f"   Final Price: ${deal['final_price']:,.2f}/unit")
            print(f"   Original Bid: ${deal['original_bid_price']:,.2f}/unit")
            print(f"   Savings from bartering: ${deal['savings_vs_bid']:,.2f}/unit")
            print(f"   Turns to close: {deal['turns']}")
        else:
            print("\n❌ No deal reached in bilateral bartering.")

        print("\n📋 ALL BILATERAL THREADS:")
        for thread in bilateral.get("all_threads", []):
            if isinstance(thread, Exception):
                print(f"   ERROR: {thread}")
            else:
                status = "✅ DEAL" if thread.get("success") else "❌ NO DEAL"
                print(
                    f"   {thread.get('supplier_id', 'unknown')}: {status} "
                    f"(turns={thread.get('turns', 0)}, "
                    f"final=${thread.get('final_price', 'N/A')})"
                )
    else:
        print("\nℹ️  Shortlist size = 1, skipping bilateral bartering.")
