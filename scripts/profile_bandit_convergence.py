...
from mesh.neuro.bandits import BanditContext, LinUCBBandit, NegotiationStrategy
...
def run_convergence_profile(num_rounds: int = 1000) -> dict:
    ...
    for w in range(num_windows):
        ...
        counts = dict.fromkeys(NegotiationStrategy, 0)
...
def print_convergence_report(results: dict) -> None:
    ...
    print("-" * 70)
    print(
        f"{'Window':<8} | {'AGGRESSIVE_ANCHOR':>18} | {'BALANCED_CONCESSION':>18} | "
        f"{'PAYMENT_TERMS':>14} | {'RISK_AWARE':>12} | {'RELATIONSHIP':>12}"
    )
    print(
        f"{'':<8} | {'':>18} | {'':>18} | {'TRADE_OFF':>14} | {'PACING':>12} | {'BUILDING':>12}"
    )
    print("-" * 70)
    for i, dist in enumerate(strategy_dist):
        print(
            f"{i+1:<8} | {dist['aggressive_anchor']:>17.1f}% | "
    ...
    print("-" * 70)
    select_line = (
        f"{'select_action':<15} | {latency['select_p50']:>9.4f} | "
        f"{latency['select_p95']:>9.4f} | {latency['select_p99']:>9.4f}"
    )
    update_line = (
        f"{'update':<15} | {latency['update_p50']:>9.4f} | "
        f"{latency['update_p95']:>9.4f} | {latency['update_p99']:>9.4f}"
    )
    print(select_line)
    print(update_line)
    print()
    ...
    print("-" * 70)
    print(
        f"{'Strategy':<28} | {'Count':>6} | {'Total Reward':>12} | "
        f"{'Avg Reward':>10} | {'Theta Norm':>10}"
    )
    print("-" * 70)
    for strategy, stats in action_stats.items():
        print(
            f"{strategy:<28} | {stats['count']:>6} | {stats['total_reward']:>12.4f} | "
    ...
    try:
        ...
        counts_match = all(
            original_stats[a]["count"] == loaded_stats[a]["count"] for a in original_stats
        )
        rewards_match = all(
