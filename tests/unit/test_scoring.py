...
def test_rank_bids(evaluator):
    bids = [
        BidPayload(
            session_id="t", supplier_id="CheapSlow", unit_price=300.0,
            lead_time_days=60, carbon_footprint_kg=1800.0,
            reliability_score=0.5, bid_bond_amount=100.0, delivery_date="2026-08-15"
        ),
        BidPayload(
            session_id="t", supplier_id="ExpensiveFast", unit_price=500.0,
            lead_time_days=15, carbon_footprint_kg=0.0,
            reliability_score=1.0, bid_bond_amount=100.0, delivery_date="2026-08-15"
        ),
    ]
    ranked = evaluator.rank_bids(bids, market_spot_price=500.0, target_lead_time=30, material="steel")
